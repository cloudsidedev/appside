...
`config.yml` holds your default config (default tenant, environment) so you do not
...
`playbooks` this is where all playbooks are placed, you can contribute to them visiting
    the repository: [Appflow-Playbooks](https://github.com/ttssdev/appflow-playbooks)
    To keep them updated, you can just type `appflow update`

`tenant` this is where all your tenants (you can have as many as you wish) will be placed.
...
`vault` this will hold your passwords to decrypt your intentories (`appflow decrypt`, `appflow encrypt`)
